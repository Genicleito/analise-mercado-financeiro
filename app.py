...
if ticker_sb:
    ...
    st.metric(label=f"Preço Predito [Erro Médio Absouto = {mae(X_test, Y)}]:", value=f"R$ {round(pred.iloc[0], 2)}", delta=f"{round((pred.iloc[0] - df_pred['close'].iloc[-1]) / df_pred['close'].iloc[-1] * 100, 2)}%")
